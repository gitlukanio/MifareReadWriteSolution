...
namespace ConsoleACR122U_2 // 
{
    public class cardConnector
    {
        ...
        private string LoadAuthkeyToAPDU(string Key)
        {
            ClearBuffers();
            SendBuff[0] = 0xFF;                         // CLA
            SendBuff[1] = 0x82;                         // INS: for stored key input
        ...
        public string authBlock(int BlockNumber, int KeyType, string Key) // function na pang authenticate dahil lumilipat tyo ng sector
        {
            LoadAuthkeyToAPDU(Key);
            ...
            SendBuff[4] = (byte)(0x60 + KeyType);                         // P3/LC: for stored key input
            SendBuff[5] = 0x00;                         // Byte 1: version number
